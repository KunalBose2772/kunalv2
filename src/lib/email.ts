...
export async function sendEmail(opts: SendEmailOptions): Promise<boolean> {
  try {
    await transporter.sendMail({
      from: `"${process.env.PDF_COMPANY_NAME ?? "Built by Kunal"}" <${process.env.MAIL_USER}>`,
      to: opts.to,
...
export function leadThankYouTemplate(name: string, services: string[]): string {
  const co = process.env.PDF_COMPANY_NAME ?? "Built by Kunal";
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    ...
    .header { background: linear-gradient(135deg, #3b82f6, var(--accent-global)); padding: 32px; text-align: center; }
    ...
    .cta a { background: linear-gradient(135deg, #3b82f6, var(--accent-global)); color: #fff; padding: 12px 28px; border-radius: 8px; text-decoration: none; font-weight: 600; }
...
<body>
  <div class="container">
    ...
    <div class="body">
      ...
      <div class="cta">
        <a href="${process.env.PDF_COMPANY_WEBSITE ?? "https://builtbykunal.online"}">Visit Our Website</a>
      </div>
    </div>
    <div class="footer">
...
<head><meta charset="utf-8" /><style>
  body { font-family: Arial, sans-serif; background: #f4f4f4; margin: 0; padding: 20px; }
  .container { max-width: 600px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; }
  .header { background: linear-gradient(135deg, #3b82f6, var(--accent-global)); padding: 20px; text-align: center; }
  .header h1 { color: #fff; margin: 0; font-size: 20px; }
  .body { padding: 32px; }
