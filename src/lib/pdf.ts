...
export async function generateQuotationPDF(quotation: Quotation): Promise<Buffer> {
  ...
  const co = {
    name:    process.env.PDF_COMPANY_NAME    ?? "Built by Kunal",
    email:   process.env.PDF_COMPANY_EMAIL   ?? "kunal@builtbykunal.online",
    phone:   process.env.PDF_COMPANY_PHONE   ?? "+91 9876543210",
    address: process.env.PDF_COMPANY_ADDRESS ?? "India",
    website: process.env.PDF_COMPANY_WEBSITE ?? "https://builtbykunal.online",
  };
