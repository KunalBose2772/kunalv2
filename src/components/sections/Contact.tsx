...
export default function Contact() {
  ...
  useEffect(() => {
    const checkPlannedScope = () => {
      const saved = localStorage.getItem("builtbykunal_planned_scope");
      if (saved) {
        try {
          ...
          localStorage.removeItem("builtbykunal_planned_scope");
        } catch (e) { console.error("Error parsing planned scope", e); }
      }
    };
    checkPlannedScope();
    window.addEventListener("builtbykunal_scope_locked", checkPlannedScope);
    return () => window.removeEventListener("builtbykunal_scope_locked", checkPlannedScope);
  }, []);
    ...
    <section className="border-t relative overflow-hidden bg-slate-950 border-white/[0.06] pb-0" id="contact">
      ...
      <div className="relative w-full z-0">
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-slate-950/90 backdrop-blur-sm px-5 py-2.5 rounded-full border border-white/[0.14] shadow-xl whitespace-nowrap">
          ...
          <span className="text-xs font-bold text-white tracking-wide">Built by Kunal — {settings.location}</span>
        </div>
        <iframe
