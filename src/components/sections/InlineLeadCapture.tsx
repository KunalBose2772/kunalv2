...
export default function InlineLeadCapture() {
  ...
  return (
    <section className="relative py-24 overflow-hidden bg-gradient-to-br from-slate-950 via-[#160b29] to-slate-955 border-b border-white/[0.06]" id="lead-form-section">
      ...
      <div className="absolute top-0 right-0 w-[55%] h-[55%] bg-[radial-gradient(circle,_rgba(var(--accent-global-rgb), 0.12)_0%,_transparent_70%)] blur-[100px] pointer-events-none z-0" />
          ...
          <div className="lg:col-span-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-[0_24px_50px_rgba(15,23,42,0.06)] relative text-slate-800">
              ...
              {sent ? (
                ...
                >
                  <div className="w-16 h-16 rounded-2xl bg-[var(--accent-global)] flex items-center justify-center mx-auto shadow-lg shadow-purple-500/20">
                  ...
                  {leadId && (
                    <div className="inline-block px-4 py-1.5 bg-slate-50 rounded-xl border border-slate-200">
                      <p className="text-[10px] text-slate-500 font-mono">Reference: <span className="text-[var(--accent-global)] font-bold">{leadId}</span></p>
                    </div>
                  )}
                  <button
                    onClick={() => { setSent(false); setLeadId(null); }}
                    className="text-xs sm:text-sm font-bold text-[var(--accent-global)] hover:text-[var(--accent-global-hover)] transition-colors block mx-auto pt-2 font-sora"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-5 font-sora">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        <User className="w-3.5 h-3.5 text-[var(--accent-global)]" /> Full Name *
                      </label>
                      <input
                        type="text"
                        required
                        ...
                        className="w-full h-11 px-4 bg-slate-50 border border-slate-200 focus:border-[var(--accent-global)] focus:bg-white focus:ring-2 focus:ring-[var(--accent-global)]/10 rounded-xl text-xs text-slate-800 placeholder-slate-400 outline-none transition-all font-medium tracking-tight"
                      />
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        <Mail className="w-3.5 h-3.5 text-[var(--accent-global)]" /> Work Email *
                      </label>
                      <input
                        type="email"
                        required
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        <Phone className="w-3.5 h-3.5 text-[var(--accent-global)]" /> Phone Number *
                      </label>
                      <input
                        type="tel"
                        required
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                        <FileText className="w-3.5 h-3.5 text-[var(--accent-global)]" /> Service Interested *
                      </label>
                      <div className="relative">
                        <select
                          ...
                          className="w-full h-11 pl-4 pr-10 bg-slate-50 border border-slate-200 focus:border-[var(--accent-global)] rounded-xl text-xs text-slate-700 outline-none transition-all font-semibold cursor-pointer appearance-none tracking-tight"
                        >
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[9px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                      <HelpCircle className="w-3.5 h-3.5 text-[var(--accent-global)]" /> Project Goals & Requirements
                    </label>
                    <textarea
                      rows={3}
                      value={form.message}
                      ...
                      className="w-full p-3.5 bg-slate-50 border border-slate-200 focus:border-[var(--accent-global)] focus:bg-white rounded-xl text-xs text-slate-800 placeholder-slate-400 outline-none transition-all font-medium resize-none leading-relaxed tracking-tight"
                    />
                  </div>
                  ...
                  <button
                    type="submit"
                    disabled={sending}
                    className="w-full h-12 rounded-xl font-bold text-xs uppercase tracking-widest text-white bg-[var(--accent-global)] hover:bg-[var(--accent-global-hover)] transition-all shadow-md shadow-purple-650/10 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60 hover:-translate-y-0.5 duration-250"
                  >
                    {sending ? (
