...
function LogoMark({ size = 68 }: { size?: number }) {
  return (
    <img
      ...
      alt="Built by Kunal"
      style={{ height: `${size}px`, width: "auto", display: "block", objectFit: "contain", maxWidth: "260px" }}
  ...
  return (
    <footer className="relative overflow-hidden bg-slate-950 border-t border-white/[0.08] text-white">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(var(--accent-global-rgb), 0.25) 0%, rgba(var(--accent-global-rgb), 0.1) 45%, transparent 80%)",
        }}
      />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at bottom right, rgba(var(--accent-global-rgb), 0.12) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.04]"
        style={{
          backgroundImage: "radial-gradient(circle, var(--accent-global-light) 1px, transparent 1px)",
          backgroundSize: "32px 32px",
        }}
      />

      {/* Top neon border glow */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[var(--accent-global)]/70 to-transparent" />
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-16 relative z-10">
        ...
        <div className="mb-16 p-8 sm:p-10 rounded-[32px] bg-white/[0.02] border border-white/[0.06] backdrop-blur-md relative overflow-hidden group/cta">
          <div className="absolute -top-12 -right-12 w-64 h-64 rounded-full blur-[80px] opacity-25 pointer-events-none bg-[var(--accent-global)]" />

          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 relative z-10">
            <div className="text-center lg:text-left space-y-2">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest text-[var(--accent-global-light)] bg-[var(--accent-global)]/10 border border-[var(--accent-global)]/20">
                <Sparkles className="w-3.5 h-3.5 text-[var(--accent-global-light)]" /> Digital Transformation
              </span>
              <h3 className="text-2xl sm:text-3xl font-extrabold font-sora tracking-tight">
                Ready to build something <span className="text-[var(--accent-global-light)]">extraordinary</span>?
              </h3>
            ...
            <div className="flex flex-wrap justify-center gap-4 shrink-0">
              <button
                ...
                className="h-12 px-7 text-white font-extrabold text-xs uppercase tracking-widest rounded-full transition-all duration-300 flex items-center gap-2 cursor-pointer shadow-lg hover:shadow-purple-500/25 bg-[var(--accent-global)] hover:bg-[var(--accent-global-hover)] hover:-translate-y-0.5 active:translate-y-0"
              >
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="space-y-4">
              ...
              <div className="inline-flex items-center gap-2 text-[10px] font-extrabold text-slate-350 uppercase tracking-widest border border-white/[0.08] bg-white/[0.02] px-3.5 py-1.5 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent-global)] inline-block animate-pulse" />
            ...
            {Object.entries(footerLinks).map(([category, links]) => (
              <div key={category} className="space-y-5">
                <h4 className="text-white font-extrabold text-[12px] uppercase tracking-widest flex items-center gap-2">
                  <span className="w-1 h-3.5 rounded-full bg-[var(--accent-global)] inline-block shadow-[0_0_8px_var(--accent-global)]" />
                  {category}
                </h4>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-3.5">
              <a href={`mailto:${settings.email}`}
                className="flex items-center gap-3 text-[13px] text-slate-200 hover:text-[var(--accent-global-light)] transition-colors duration-250 group">
                <div className="w-8 h-8 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center shrink-0 group-hover:border-[var(--accent-global)]/40 group-hover:bg-[var(--accent-global)]/10 transition-all">
                  <Mail className="w-3.5 h-3.5 text-[var(--accent-global-light)]" />
                </div>
              ...
              <a href={`tel:${settings.phone.replace(/\s+/g, "")}`}
                className="flex items-center gap-3 text-[13px] text-slate-200 hover:text-[var(--accent-global-light)] transition-colors duration-250 group">
                <div className="w-8 h-8 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center shrink-0 group-hover:border-[var(--accent-global)]/40 group-hover:bg-[var(--accent-global)]/10 transition-all">
                  <Phone className="w-3.5 h-3.5 text-[var(--accent-global-light)]" />
                </div>
              ...
              <div className="flex items-start gap-3 text-[13px] text-slate-200">
                <div className="w-8 h-8 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center shrink-0">
                  <MapPin className="w-3.5 h-3.5 text-[var(--accent-global-light)]" />
                </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-white/[0.06]">
              {socials.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  aria-label={label}
                  className="w-9 h-9 rounded-xl flex items-center justify-center border border-white/[0.08] bg-white/[0.03] text-slate-300 hover:text-white hover:border-[var(--accent-global)]/40 hover:bg-[var(--accent-global)]/15 transition-all duration-300 hover:scale-105 active:scale-95"
                >
        ...
        <div className="border-t border-white/[0.06] py-6 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-slate-300">
            <Globe2 className="w-4 h-4 text-[var(--accent-global-light)]" />
            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--accent-global-light)]">Development Hubs</span>
          </div>
