...
const stats = [
  { icon: TrendingUp, value: "150+", label: "Projects Shipped", color: "var(--accent-global)" },
  ...
  return (
    <div className="min-h-screen bg-[#fafbfc]">
      ...
      <PageHero
        ...
        description={`Built by Kunal was founded in ${COMPANY.founded} with a singular mission: to make enterprise-grade software accessible to every ambitious business — not just the Fortune 500.`}
      ...
      <section className="py-24 bg-white border-b border-slate-200/60 relative overflow-hidden">
        <div className="absolute top-[10%] right-[-5%] w-[30%] h-[30%] bg-[radial-gradient(circle,_rgba(var(--accent-global-rgb), 0.03)_0%,_transparent_75%)] blur-[90px] pointer-events-none" />
          ...
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                ...
                {[
                  ...
                  <div key={idx} className="flex gap-3.5 items-start">
                    <div className="w-6 h-6 rounded-full bg-purple-50 border border-purple-100 flex items-center justify-center shrink-0 mt-0.5">
                      <CheckCircle2 className="w-3.5 h-3.5 text-[var(--accent-global)]" />
                    </div>
                    <div>
            ...
            <div className="lg:col-span-6 relative">
              ...
              <div className="relative rounded-2xl overflow-hidden border border-slate-200 shadow-2xl bg-white">
                <div className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                  ...
                  <div className="mx-auto w-[60%] h-4 rounded bg-white border border-slate-200 text-[8px] text-slate-400 flex items-center justify-center font-mono">
                    builtbykunal.online/mission
                  </div>
                </div>
                <div className="relative aspect-[16/10] bg-slate-50">
                  <Image
                    src="/images/careers/team_collab.png"
                    alt="Built by Kunal team collaborating on code"
                    fill
              ...
              <div className="absolute bottom-[-16px] left-[-16px] bg-white border border-slate-200 rounded-xl p-3.5 shadow-xl hidden sm:flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-[var(--accent-global)]/10 flex items-center justify-center text-[var(--accent-global)]">
            ...
            <div className="lg:col-span-6 order-2 lg:order-1 relative">
              ...
              <div className="relative rounded-2xl overflow-hidden border border-slate-200 shadow-2xl bg-white">
                <div className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                  ...
                  <div className="mx-auto w-[60%] h-4 rounded bg-white border border-slate-200 text-[8px] text-slate-400 flex items-center justify-center font-mono">
                    builtbykunal.online/vision
                  </div>
                </div>
                <div className="relative aspect-[16/10] bg-slate-50">
                  <Image
                    src="/images/careers/workstation.png"
                    alt="Built by Kunal clean developer workstation"
                    fill
                    className="object-cover"
                    sizes="(max-width: 1024px) 100vw, 600px"
                  />
            ...
            {[...Array(2)].map((_, setIdx) => (
              <div key={setIdx} className="flex gap-6">
                {teamMembers.map((member, i) => (
                  <div
                    key={`${setIdx}-${i}`}
                    className="w-[300px] shrink-0 bg-slate-50 border border-slate-200/80 rounded-3xl hover:shadow-xl hover:bg-white hover:border-[var(--accent-global)]/25 transition-all duration-300 flex flex-col justify-between overflow-hidden"
                  >
                    <div>
                      {/* Avatar Image Frame */}
                      ...
                      <div className="p-5 pb-0">
                        ...
                        <p className="text-[11px] font-bold text-[var(--accent-global)] uppercase tracking-wider mb-2.5">
                          {member.role}
                        </p>
                    ...
                    <div className="flex items-center justify-between p-5 pt-3.5 border-t border-slate-200/50">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Built by Kunal Partner
                      </span>
                      <div className="flex gap-2">
                        {member.linkedin && member.linkedin !== "#" && member.linkedin.trim() !== "" && (
                          <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-full bg-purple-50 text-[var(--accent-global)] hover:bg-[var(--accent-global)] hover:text-white flex items-center justify-center transition-all duration-200 shadow-sm" aria-label="LinkedIn">
                            <FaLinkedinIn className="w-3.5 h-3.5" />
                          </a>
                        )}
                        {member.twitter && member.twitter !== "#" && member.twitter.trim() !== "" && (
                          <a href={member.twitter} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-full bg-purple-50 text-[var(--accent-global)] hover:bg-[var(--accent-global)] hover:text-white flex items-center justify-center transition-all duration-200 shadow-sm" aria-label="Twitter">
                            <FaTwitter className="w-3.5 h-3.5" />
                          </a>
                        )}
                        {member.github && member.github !== "#" && member.github.trim() !== "" && (
                          <a href={member.github} target="_blank" rel="noopener noreferrer" className="w-7 h-7 rounded-full bg-purple-50 text-[var(--accent-global)] hover:bg-[var(--accent-global)] hover:text-white flex items-center justify-center transition-all duration-200 shadow-sm" aria-label="GitHub">
